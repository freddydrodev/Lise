...
if(isset($_POST['register'])){
  ...
  if(!preg_match('/^[a-zA-Z ]{5,100}$/', $fn)) {
      ...
      // echo '<div class="alert alert-danger p-2" role="alert"><strong>Full Name Wrong Format!</strong> 5 - 100 letters & spaces only</div>';
  }
  ...
  if (strlen($ps) < 6) {
      ...
      // echo '<div class="alert alert-danger p-2" role="alert"><strong>Password!</strong> 6 character minimum</div>';
  }

  if($ps !== $cn){
      $correct = false;
      bootstrapNotify('Confirm: Les Mots de pass ne concorde pas');
      // echo '<div class="alert alert-danger p-2" role="alert"><strong>Password!</strong> Password and Confim do not match</div>';
  }
  ...
  if($correct){
    ...
    if($add->execute(array(ucwords($fn), $un, sha1($ps)))){
      ...
      bootstrapNotify();
        echo '<div class="alert alert-danger p-2" role="alert"><strong>Error</strong> Something went wrong</div>';
    }
    $add->closeCursor();
  }
  else {
